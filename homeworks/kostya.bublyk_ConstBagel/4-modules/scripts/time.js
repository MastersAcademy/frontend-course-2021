 ...
 */
 function getMatrixOfCurrentMonth (pickedDate) {
    ...
    return Array(fullWeeksOfPickedMonth.length / amountDaysPerWeek).fill(null)
        .map(() => fullWeeksOfPickedMonth.splice(0, amountDaysPerWeek)
            .filter(Boolean));
}
...
export function getFridaysOfMonth (date) {
    return getMatrixOfCurrentMonth(date)
        .reduce((result, week) => {
...
export function fullWeeksNumberInMonth (date) {
    return getMatrixOfCurrentMonth(date)
        .filter((week) => week.length === 7).length;
}
